...
> 免责说明：Odin1 输出的 odometry / 定位结果仅供客户上层应用参考，我司不对基于该数据的判断与运行结果承担责任。在无人机、特种作业等对定位连续性要求较高的场景中，里程计漂移可能导致炸机、设备损坏等后果，相关风险需由使用方自行评估。我们无法保证 Odin1 适配所有场景，请在直接使用其输出结果前充分完成场景化测试，并做好安全冗余设计（如失效保护、多传感器融合）。
