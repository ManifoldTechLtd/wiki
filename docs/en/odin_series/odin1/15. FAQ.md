...
1. Possible hardware fault on a single LiDAR (swapping cables doesn't help, but swapping the device does → the device may have a hardware fault and may need service).
...
**Answer:** Yes, this existed in early versions. It was fixed in the 0.11.0 series with NTP smoothing and a redesign of the driver-side timestamps. Please upgrade to the latest firmware and driver and re-configure.
...
### ❓ Q5.1 Inaccurate point-cloud ranging at close range (within 0.4 m)?

**Answer:** Within 0.4 m, noticeable point dragging may occur. The recommendation is to **use `cloud_raw` for close-range obstacle avoidance and `cloud_slam` for mapping/localization**. If your robot has near-field obstructions during installation (such as bumpers or impact rings), tilt the sensor slightly forward or move it to avoid structural interference.
...
- **Things to check:**
  ...
  2. When scanning with Q9000, keep the front camera angle and height as aligned as possible with how Odin1 will actually be mounted.
...
### ❓ Q6.3 Can the LiDAR be mounted vertically for relocalization?
...
**Answer:** Odin1 provides a **14.5 Hz mode**, which can increase the scanning frame rate. However, because Odin is a flash solid-state LiDAR, each scan captures the point cloud from a single position, so in a static state it does not increase resolution or scanning density.
...
**Answer:**
- **Different principle:** mechanical LiDARs mainly rely on mechanical rotation to achieve 360-degree scanning, while Odin1 is a pure solid-state LiDAR with global exposure.
- **Different structure:** no mechanical rotation, with better vibration resistance (see Section 8).
