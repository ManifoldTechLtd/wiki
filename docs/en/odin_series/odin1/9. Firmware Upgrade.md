...
> ⚠️ **Important: upgrading to firmware 0.13.1 is currently NOT recommended**
>
> Testing has revealed the following two known issues in firmware **0.13.1**, which are being fixed:
>
> 1. When the timestamp source is set to `host_time2`, **it fails to sync with the host system time** (confirmed bug).
> 2. Noticeably higher point-cloud noise.
>
> Recommendation: **stay on firmware 0.12.0 for now** and wait for the next patched release before upgrading. If you have already upgraded and need to roll back, please contact after-sales support.
