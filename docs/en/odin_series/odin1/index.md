---
layout: default
title: Odin1 (EN)
nav_exclude: true
search_exclude: true
permalink: /en/odin_series/odin1/
---

# Odin1

| Feature | Specification |
|:---|:---|
| **Ultra-high-performance perception** | 70 m (90 % reflectivity) / 30 m (10 % reflectivity) ultra-long range · 120° × 90° ultra-wide field of view · up to 700 k points/sec high-density point cloud · color global-shutter imaging |
| **Ultra-robust spatial memory** | ±5 cm + 1 % centimeter-level global localization · up to 400 Hz pose update · self-adaptive in low-light / glass / water-surface and other complex environments |
| **Compact all-in-one industrial design** | ~280 g ultra-light · 100 × 62 × 43 mm compact size · IP66 dust-/water-proof · 12–24 V wide-voltage input · shock-resistant up to 32 G |

---

## Documentation Sections

- [Odin1 User Manual](./Cover.html)
- [Reading Tips](./Reading%20Tips.html)
- [1. Product Overview](./1.%20Product%20Overview_.html)
- [2. Hardware Description](./2.%20HW%20Description_.html)
- [3. Installation Guide](./3.%20Installation%20Guide_.html)
- [4. Power & Connection](./4.%20Power%20connection.html)
- [5. Data Output](./5.%20Data%20output_.html)
- [6. Mapping & Relocalization API Workflow](./6.%20map%20and%20relocalizaiton%20api.html)
- [7. Odin1 Quick Start (Ubuntu)](./7.%20Odin1%20Quick%20start%20for%20ubuntu.html)
- [8. OdinViewer Quick Start (Windows)](./8.%20OdinViewer%20Quick%20start%20for%20windows.html)
- [9. Firmware Upgrade](./9.%20Firmware%20Upgrade.html)
- [10. Relocalization User Guide](./10.%20Relocalization%20Usr%20Guide.html)
- [11. Relocalization Map Acquisition Guide](./11.%20Relocalization%20Map%20Acquisition%20Guide.html)
- [12. Time Synchronization](./12.%20Time%20Synchronization.html)
- [13. Maintenance](./13.%20Maintenance.html)
- [14. Technical Specifications](./14.%20Technical%20Specifications_.html)
- [15. FAQ](./15.%20FAQ.html)
- [Follow Us](./Footer.html)

---

> ℹ️ The English version is being translated chapter by chapter. Pages still showing a "translation in progress" notice will fall back to the Chinese version — you can also switch to Chinese any time via the toggle at the top-right corner.
