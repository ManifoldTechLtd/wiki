---
title: "联系+关注我们"
search_exclude: true  # 从搜索中排除
parent: 更多信息
nav_order: 1
---

#  关注我们

## 1. 获取最新动态资讯


<div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 40px; max-width: 500px; margin: 40px auto; text-align: center;">

  <div>
    <img src="../../../01_series/assets/img/manifold_account.jpg"
         alt="留形科技公众号"
         width="180"
         height="180"
         style="object-fit: contain;">
    <p><strong>留形科技公众号</strong></p>
  </div>

  <div>
    <img src="../../../01_series/assets/img/channel.jpg"
         alt="案例视频号"
         width="180"
         height="180"
         style="object-fit: contain;">
    <p><strong>案例视频号</strong></p>
  </div>

  <div>
    <img src="../../../01_series/assets/img/xiaoshongshu.jpg"
         alt="小红书"
         width="180"
         height="180"
         style="object-fit: contain;">
    <p><strong>小红书</strong></p>
  </div>

  <div>
    <img src="../../../01_series/assets/img/bilibili.jpg"
         alt="bilibili"
         width="180"
         height="180"
         style="object-fit: contain;">
    <p><strong>Bilibili</strong></p>
  </div>

</div>



## 2. 扫码步骤
1. 打开微信"扫一扫"
2. 对准上方二维码
3. 点击"关注公众号"

## 3. 关注福利
- ✅ 最新固件推送
- ✅ 技术文档更新
- ✅ 线上技术支持
- ✅ 用户社区交流

<div align="center">

{{ site.time | date: "%Y-%m-%d" }}

</div>